...
sp_500_stock_key_serializer = StringSerializer('utf_8')
sp_500_stock_value_serializer = load_avro_serializer_from_file('sp500_VALUE.avsc')
...
def send_csv_data_avro(topic, filename, key_serializer, value_serializer):
    ...
    with open(filename, 'r') as f:
        ...
        for row in reader:
            key = str(uuid.uuid4()) if topic == 'changeDC' else row.get('sessionid', str(uuid.uuid4()))
            data = {
                k: str(v) if k in ['Adj Close', 'Close', 'High', 'Low', 'Open', 'Volume'] else str(v) if k == 'Date' else v
...
def run_producer_for_sp500_stock():
    send_csv_data_avro('changeDC', 'sp500_stocks_aapl.csv', sp_500_stock_key_serializer, sp_500_stock_value_serializer)    
